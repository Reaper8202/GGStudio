...
`WeaponDefinition` includes `mountType`, `aimMode`, arc, damage, fire rate, ammunition/power use, recoil, projectile speed, and range. `aimMode` is either `'auto'` or `'manual'`:

- `turret` is an auto-aiming, 360° turret.
- `cannon-heavy` is a manually aimed, 120° heavy cannon.
