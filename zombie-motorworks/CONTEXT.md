...
| `scraprig.onboarding.v1`                       | `src/app/App.ts`                                          | First-car onboarding stage: `garage`, `combat`, `done`, or `skipped`                                                           |
| `scraprig.help-seen`                           | `src/editor/ui.ts`                                        | Presentation-only Help acknowledgement                                                                                         |
...
- First-car onboarding runs only for a New Game. App starts the bay with the
  Chassis Core and stages the old free starter-body pieces in Inventory;
  `core/tutorial.ts` owns the exact part/cell/orientation recipe. Editor blocks
  every mutation except Store-to-cell dragging and prescribed quarter-turns,
  then requires buying and placing one Turret before Fight. Skip restores the
  canonical prebuilt starter body, clears staged stock, restores starting money,
  and opens the ordinary weapon prompt.
- App owns the onboarding stage across the Garage/Survival Seam. Survival pauses
  wave 1 before its countdown for driving, auto-fire, and manual-fire cards,
  then releases the normal wave after completion or skip. Tutorial state is
  presentation state and never enters Run Checkpoint or RunState.
...
| Wave balance/composition           | `src/survival/WaveManager.ts`                | `waveBalance.ts`, `zombies/zombieConfig.ts`                                                                                                                   | `unit/waves.test.ts`, `unit/zombie-balance.test.ts`                                                             |
...
| First-car onboarding               | `src/core/tutorial.ts`                       | `app/App.ts`, `editor/TutorialOverlay.ts`, `EditorMode.ts`, `ui.ts`, `survival/FirstWaveTutorial.ts`, `TouchDriveControls.ts`, `SurvivalMode.ts`, `style.css` | `unit/tutorial.test.ts`, `unit/touch-drive-controls.test.ts`, `unit/app.test.ts`                                |
