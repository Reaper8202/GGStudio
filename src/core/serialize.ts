...
const LEGACY_DEF_ID_MAPPINGS: Readonly<Record<string, string>> = {
...
const DROPPED_LEGACY_DEF_IDS = new Set(['armour-panel', 'shell-panel']);
const LEGACY_BEAM_CELLS: readonly Vec3i[] = [
  { x: 0, y: 0, z: 0 },
  { x: 0, y: 0, z: 1 },
  { x: 0, y: 0, z: 2 },
];
...
export const MIGRATIONS: Record<number, (raw: unknown) => unknown> = {
  ...
  2: (raw: unknown): unknown => {
    ...
    return {
      ...bp,
      schemaVersion: 3,
      parts: rawParts.flatMap((partRaw, index) => {
        const prefix = `parts[${index}]`;
        const part = requireRecord(partRaw, prefix);
        const defId = requireString(part.defId, `${prefix}.defId`);

        if (DROPPED_LEGACY_DEF_IDS.has(defId)) return [];
        ...
        if (defId === 'beam-long') {
          ...
          return LEGACY_BEAM_CELLS.map((localCell, cellIndex) => ({
            ...part,
            id: cellIndex === 0 ? id : `${id}${cellIndex === 1 ? 'b' : 'c'}`,
            defId: 'frame-box',
            pos: addVec(pos, rotateVec(orient, localCell)),
            orient: 0,
          }));
        }

        return [{ ...part, defId: LEGACY_DEF_ID_MAPPINGS[defId] ?? defId }];
      }),
    };
  },
};
...
function migrateToCurrent(raw: unknown): unknown {
  ...
  if (version !== CURRENT_SCHEMA_VERSION)
    throw new BlueprintFormatError(`unsupported schema version ${version}`);
  return current;
}
...
function validateBlueprint(raw: unknown): VehicleBlueprint {
  ...
  const parts: PlacedPart[] = partsRaw.map((partRaw, index) =>
    validatePart(partRaw, index, seenIds),
  );
...
function validatePart(
  ...
  const id = requireString(part.id, `${prefix}.id`);
  if (seenIds.has(id))
    throw new BlueprintFormatError(`duplicate part id: ${id}`);
  seenIds.add(id);
...
function requireSchemaVersion(raw: unknown): number {
  ...
  if (!Number.isInteger(version))
    throw new BlueprintFormatError('schemaVersion must be an integer');
  return version;
}
...
function requireVec3i(raw: unknown, path: string): Vec3i {
  ...
  if (
    !Number.isInteger(out.x) ||
    !Number.isInteger(out.y) ||
    !Number.isInteger(out.z)
  ) {
